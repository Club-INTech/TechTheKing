...
void Clavier::begin(int dataPin, int clockPin, bool kybdLites) {
...
void Clavier::begin(int dataPin) {
  // Prepare the global variables
  ps2Keyboard_Lites         = true;
  ps2Keyboard_DataPin       = dataPin;
  ps2Keyboard_ClockPin      = 3;
  ps2Keyboard_CurrentBuffer = 0;
  ps2Keyboard_CharBuffer    = 0;
  ps2Keyboard_BufferPos     = 0;
  ps2Keyboard_shift         = false;
...
void Clavier::begin( void ) {
  // Prepare the global variables
  ps2Keyboard_Lites         = true;
  ps2Keyboard_DataPin       = 4;
  ps2Keyboard_ClockPin      = 3;
  ps2Keyboard_CurrentBuffer = 0;
  ps2Keyboard_CharBuffer    = 0;
  ps2Keyboard_BufferPos     = 0;
...
byte Clavier::read() {
  ...
  case 0x4C: result = ps2Keyboard_shift? ':' : ';'; break;
...
uint8_t Clavier::confirmation(uint8_t touche)
{
	uint8_t retour;
	uint8_t tempo = 0; 
	while( tempo <= 100 && !(this->available()) )
	{
		_delay_ms(10);
		tempo++;
	}
	if( tempo >= 100 )
		return 0;
	if( this->lectureTouche( &retour ) >= 0 )
	{
		if( retour == touche )
		return 1;
	}
