...
JNIEXPORT jstring JNICALL Java_intech_detection_PictureTaker_trouverPions(JNIEnv* env, jobject thiz,jint morphRows,jint morphCols){
	int cfg_morphRows = 8 ;
	int cfg_morphCols = 8 ;
	int cfg_teinteMin = 25;
	int cfg_teinteMax = 45;
	int cfg_sat=80;
	...
	LOGI("Creation matrice homographie");
	homogMatrix = initHomogMatrix(pImage,vue) ;

	IplImage *imgBin ;

	imgBin = binariseHSV(pImage,cfg_teinteMin,cfg_teinteMax,cfg_sat, cfg_morphRows, cfg_morphCols);
	...
	LOGI("Binarisation");
	IplImage *imgBin ;
	binarisation_nvg(pImage,imgBin,cfg_seuil) ;

	// Suppression des pixels isol�s
	LOGI("Ouverture");
	ouverture(imgBin,cfg_morph_cols,cfg_morph_rows) ;
	*/

	// D�tection des pions
	LOGI("Detection des pions");
	std::list<Point> listeCentres = detectionEllipse(imgBin);
	...
	if(!listeCentres.empty()){
			...
			CvPoint center;
			center.x = cvRound(it->y);
			center.y = cvRound(it->x);
